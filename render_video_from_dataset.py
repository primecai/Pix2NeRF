...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_obj', type=int, default=5000)
    parser.add_argument('--output_dir', type=str, default='')
    parser.add_argument('--max_batch_size', type=int, default=60000)
    ...
    dataloader, _ = datasets.get_dataset(opt.curriculum,
    ...
    for seed in tqdm(range(opt.num_obj)):
        real_imgs, _ = next(dataloader_iterator)
        real_imgs = real_imgs.to(device, non_blocking=True)
        frames = []
        z, pos = encoder(real_imgs.to(device), alpha=1.0)
        output_name = f'{seed}.mp4'
        this_output_dir = os.path.join(opt.output_dir, str(seed))
        os.makedirs(this_output_dir, exist_ok=True)
        writer = skvideo.io.FFmpegWriter(os.path.join(this_output_dir, output_name), outputdict={'-pix_fmt': 'yuv420p', '-crf': '21'})
        ...
        save_image(real_imgs, os.path.join(this_output_dir, f'input.png'), normalize=False)

        clip = (VideoFileClip(os.path.join(this_output_dir, output_name)))
        gif_name = f'{seed}.gif'
        clip.write_gif(os.path.join(this_output_dir, gif_name))